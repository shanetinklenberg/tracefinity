...
function CapturePageInner() {
  ...
  useEffect(() => {
    ...
    console.log('[capture] polling session', waitingSessionId)

    pollRef.current = setInterval(() => {
      getSessionApi(waitingSessionId)
        .then(session => {
          console.log('[capture] poll response', {
            tools_saved_at: session.tools_saved_at,
          ...
          if (session.next_session_id) {
            console.log('[capture] following next_session_id', session.next_session_id)
            if (pollRef.current) clearInterval(pollRef.current)
            setSessionId(session.next_session_id)
            setState({ status: 'idle' })
            return
          ...
          if (session.tools_saved_at) {
            console.log('[capture] tools saved, showing complete')
            if (pollRef.current) clearInterval(pollRef.current)
    ...
    return () => {
      if (pollRef.current) {
        console.log('[capture] stopping poll')
